...
namespace QuanLyNganHang.DataAccess
{
    public class AccountDataAccess
    {
        ...
        public bool FreezeAccountById(int accountId)
        {
            try
            {
                using (var conn = Database.Get_Connect())
                {
                    ...
                    string sql = "UPDATE ADMIN_NGANHANG.accounts SET status = 2 WHERE account_id = :id";
                    using (var cmd = new OracleCommand(sql, conn))
                    {
        ...
        public DataTable GetFrozenAccounts()
        {
            ...
            {
                throw new Exception("Lỗi khi lấy tài khoản đóng băng: " + ex.Message);
            }
            return dt;
        }
        ...
        public DataTable GetActiveAccounts()
        {
            ...
            {
                throw new Exception("Lỗi khi lấy tài khoản hoạt động: " + ex.Message);
            }
        }
        ...
        public DataTable FindActiveAccountsByEncryptedInput(string encryptedInput)
        {
            ...
            {
                throw new Exception("Lỗi tìm tài khoản: " + ex.Message);
            }
            return dt;
        }

        public DataTable GetActiveAccountsByCustomerId(int customerId)
        {
            DataTable dt = new DataTable();
            try
            {
                using (var conn = Database.Get_Connect())
                {
                    ...
                    string sql = @"
                        ...
                        FROM ADMIN_NGANHANG.accounts a
                        JOIN ADMIN_NGANHANG.customers c ON a.customer_id = c.customer_id
                        WHERE a.status = 1 AND c.customer_id = :cid";
            ...
            {
                throw new Exception("Lỗi khi lấy tài khoản hoạt động theo ID khách hàng: " + ex.Message);
            }
